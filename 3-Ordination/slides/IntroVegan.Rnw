...
\frametitle{Topics adressed}
  \adjustbox{max height=\dimexpr\textheight-5.5cm\relax, % scale table to fit to slide
             max width=\textwidth}{
    \begin{tabular}{lcccc}
     & \multicolumn{1}{l}{\textbf{Raw data}} & \multicolumn{1}{l}{\textbf{Transformed data}} & \multicolumn{1}{l}{\textbf{Unimodal}} & \multicolumn{1}{l}{\textbf{Distance-based}} \\ 
    \textbf{Unconstrained} & \cellcolor{green} PCA & tb-PCA & CA, DCA & \cellcolor{green} PCoA, NMDS \\ 
    \textbf{Constrained} & \cellcolor{green} RDA & \cellcolor{green} tb-RDA & CCA & db-RDA \\ 
    \end{tabular}
  }
...
\frametitle{Principal Components Analysis (PCA) | What?}
  ...
  \begin{itemize}
    ...
    \item The Eigenvectors give the roration needed
