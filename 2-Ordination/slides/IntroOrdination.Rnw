...
%%% ----------------------------------------------------------------------------
\subsection{Fit environmantal variables}
\begin{frame}[fragile]
...
<<dbRDA1, warning=FALSE, eval = FALSE, message=FALSE, purl = TRUE, message = FALSE>>=
# dbRDA
...
<<dbRDA1a, warning=FALSE, echo = FALSE, message=FALSE, purl = FALSE, message = FALSE>>=
dbRDA <- capscale(Dabu ~ ., data = Denv, 
                  distance = 'bray')
@

<<dbRDA3, warning=FALSE, message=FALSE, output.lines = 6:17>>=
summary(dbRDA)
@
...
\begin{itemize}
  ...
  \item Many methods available (see ref.), also use your \emph{ecological knowledge}!
\end{itemize}
...
\begin{itemize}
  \item VIF \textgreater 20 : strong collonearity
  \item VIF \textgreater 10 : potential problems
\end{itemize}
...
\frametitle{Goodness of fit}
  \begin{itemize}
    \item \texttt{goodness()} returns the cumulative proportion of variance explained by constraints
    \item available for \emph{sites} or \emph{species}
    \item \texttt{summarize = TRUE} gives the accumulated total variance explained
  \end{itemize}
...
\frametitle{Inertia decomposition}
  \begin{itemize}
    \item \texttt{inertcomp()} decomposes variance expalained by constrained and unconstrained
  \end{itemize}
...
\subsection{Permutation Tests (I)}
\begin{frame}
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item $$p = \frac{(No.~of~F_{perm} \ge F))}{Total~No.~of~F_{perm} + 1}$$
  \end{enumerate}
